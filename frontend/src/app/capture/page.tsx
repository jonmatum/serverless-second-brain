    ...
    <div className="space-y-6">
        ...
        <Card className="border-green-500/50 bg-green-500/5">
          <CardContent className="p-3 sm:p-4 space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <TypeBadge type={result.node_type} />
              <StatusBadge status={result.status} />
            </div>
            <div className="flex flex-wrap gap-1.5">
              {result.tags.map((tg) => <TagBadge key={tg} tag={tg} />)}
            </div>
            <Link href={`/node?id=${result.slug}`} className="text-sm text-blue-600 dark:text-blue-400 hover:underline">
              {t("capture.view_node", locale)}
            </Link>
          </CardContent>
        </Card>
      )}
      ...
      <form onSubmit={submit} className="space-y-4">
        <div>
          ...
          />
          <p className={`mt-1 text-xs ${valid ? "text-muted-foreground" : "text-destructive"}`}>
            {t("capture.char_count", locale, { count: charCount })}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          <div>
            ...
            <Select value={type} onValueChange={(v: string | null) => v && setType(v)}>
              ...
              <SelectContent>
                {TYPES.map((tp) => (
                  <SelectItem key={tp} value={tp}>{t(`type.${tp}` as Parameters<typeof t>[0], locale)}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div>
            ...
            <Select value={lang} onValueChange={(v: string | null) => v && setLang(v as "es" | "en")}>
              <SelectTrigger className="mt-1 w-full sm:w-[120px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
